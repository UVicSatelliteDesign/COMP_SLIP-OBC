...
#define IMAGE_SIZE_1 ((size_t)(IMAGE_WIDTH * IMAGE_HEIGHT * 3)) /**< Size of the image buffer in bytes (for 640x480, JPEG, Colour). */

// image buffer size for camera2
#define IMAGE_SIZE_2 ((uint32_t)(IMAGE_WIDTH * IMAGE_HEIGHT * 3 * JPEG_COMPRESSION_RATIO)) /**< Size of the image buffer in bytes (for 640x480, JPEG, Colour). */
...
#define MAX_IMAGE_BUFFER_SIZE 320000 // 614400 bytes
...
typedef struct Camera_t {
    ...
    uint32_t* imageBuffer; /**< Pointer to the image buffer where captured images are stored. */
    ...
    // probably redundant ov5640_ctx_t ctx; /**< OV5640 context structure. */ // initialize ctx.handle with the relevant i2c bus
